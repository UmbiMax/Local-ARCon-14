...
public sealed partial class ArcaneTTSManager
{
    ...
    public void Initialize()
    {
        ...
        _cfg.OnValueChanged(ACCVars.TTSMaxCache, val =>
        {
            _maxCachedCount = val;
            ResetCache();
        }, true);
        _cfg.OnValueChanged(ACCVars.TTSApiUrl, v => _apiUrl = v, true);
        _cfg.OnValueChanged(ACCVars.TTSApiToken, v =>
        {
            _apiToken = v;
    ...
    public async Task<byte[]?> ConvertTextToSpeech(string speaker, string text, string? effect = null)
    {
        ...
        try
        {
            ...
            var timeout = _cfg.GetCVar(ACCVars.TTSApiTimeout);
